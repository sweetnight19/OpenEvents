...
async function getFriendsRequest(req, res) {
  let rows = {};
  try {
    const [rows1, fields] = await conn
      .promise()
      .query(
        "SELECT DISTINCT(users.id),users.name,users.last_name,users.image,users.email FROM users INNER JOIN friends ON users.id=friends.user_id WHERE friends.status=0 AND friends.user_id_friend=? ORDER BY users.id",
        [req.USER.id]
      );
    try {
      const [rows2, fields] = await conn
        .promise()
        .query(
          "SELECT DISTINCT(users.id),users.name,users.last_name,users.image,users.email FROM users INNER JOIN friends ON users.id=friends.user_id_friend WHERE friends.status=0 AND friends.user_id=? ORDER BY users.id",
          [req.USER.id]
        );
...
async function acceptFriendsRequest(req, res, next) {
  try {
    const [resultado] = await conn
      .promise()
      .query(
        "UPDATE `friends` SET `status`=1 WHERE `user_id`=? AND `user_id_friend`= ?",
...
async function deleteFriendsRequest(req, res, next) {
  try {
    const [resultado] = await conn
      .promise()
      .query("DELETE FROM `friends` WHERE `user_id`=? AND `user_id_friend`=?", [
        req.params.id,
        req.USER.id,
      ]);
    return res.status(204).end();
